...
const ChapterMap: React.FC<ChapterMapProps> = ({
  ...
  const levelsWithPos = useMemo(() => {
    ...
    return levels.map((level, index) => {
      ...
      const xOffset = Math.sin(index / MAP_CONFIG.FREQUENCY) * MAP_CONFIG.AMPLITUDE;
  ...
  return (
    ...
    >
      ...
      <div
        className="repeating-drums-layer"
        style={{
          position: "absolute",
          top: 0,
          left: 0,
      ...
      >
        {Array.from({length: numDrums}).map((_, i) => {
          ...
          return (
            ...
            >
              <img src={bronze_drum} alt="" style={{width: "100%", animation: "drumRotate 60s linear infinite"}} />
            </div>
          );
        })}
      </div>
          ...
          {levelsWithPos.map((level, index) => {
            ...
            return (
              <path
                ...
                stroke={nextLevel.isLocked ? "var(--border-color-base)" : "var(--primary-color)"}
                strokeWidth="10"
                fill="none"
                strokeLinecap="round"
                className="path-line"
        ...
        {levelsWithPos.map((level, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Stars / Score */}
              {level.isCompleted && level.playerBestScore !== undefined && (
                <div className="level-stars-container">
                  {[1, 2, 3].map((i) => (
                    <StarFilled
                      ...
                      style={{
                        color:
                          i <= ((level.playerBestScore ?? 0) > 80 ? 3 : (level.playerBestScore ?? 0) > 50 ? 2 : 1)
                            ? "var(--gold-color)"
