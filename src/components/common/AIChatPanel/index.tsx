...
import { Input, Button, Avatar, Space, Typography, Tooltip, Spin, message as antdMessage } from 'antd';
import {
    SendOutlined,
    DeleteOutlined,
    SoundOutlined,
...
const AIChatPanel: React.FC<AIChatPanelProps> = ({
    ...
    const { chatHistory, currentCharacter, chatLoading, isTyping, error } = useAppSelector(
        (state) => state.ai
    );
        ...
        <div className={`ai-chat-panel ${isMinimized ? 'minimized' : ''}`}>
            ...
            <div className="ai-chat-panel__header">
                <Space>
                    <CommentOutlined />
                    <Text strong>
                        {currentCharacter ? `Chat với ${currentCharacter.name}` : 'AI Assistant'}
                    </Text>
                </Space>
                <Space>
            ...
            <div className="ai-chat-panel__messages">
                {chatHistory.length === 0 ? (
                    ...
                    chatHistory.map((msg) => (
                        <div
                            key={msg.id}
                            className={`ai-chat-panel__message ${msg.role === 'user' ? 'user' : 'assistant'
                                }`}
                        >
