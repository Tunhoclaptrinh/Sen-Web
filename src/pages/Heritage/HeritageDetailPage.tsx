...
const HeritageDetailPage = () => {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await favoriteService.remove("heritage_site", id);
        setIsFavorite(false);
        message.success("Đã xóa khỏi yêu thích");
      } else {
        await favoriteService.add("heritage_site", id);
        setIsFavorite(true);
    ...
    <div>
      ...
      <Card>
        <Row gutter={24}>
          <Col xs={24} md={12}>
            <img
              src={site.images && site.images.length > 0 ? site.images[0] : (site.main_image || "https://via.placeholder.com/800x600")}
              alt={site.name}
