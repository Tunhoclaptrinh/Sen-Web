...
const DetailModal: React.FC<DetailModalProps> = ({open, onCancel, record}) => {
    ...
    const fetchData = async () => {
      if (open && record?.id) {
        ...
        try {
          ...
          if (artifactsRes.data) artifactsRes.data.forEach((a: Artifact) => artifactMap.set(a.id, a));
          if (relArtifactsRes.data) relArtifactsRes.data.forEach((a: Artifact) => artifactMap.set(a.id, a));
  ...
  return (
    <Modal open={open} onCancel={onCancel} title={record.name} width={1000} footer={null}>
        ...
        <Tabs.TabPane tab="Thông tin chung" key="info">
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="Loại hình">
              <Tag color="blue">
                {HeritageTypeLabels[record.type as HeritageType]?.toUpperCase() || record.type?.toUpperCase()}
              </Tag>
            </Descriptions.Item>
          ...
          {(record.gallery || []).length > 0 || (record.images || []).length > 0 ? (
            <div style={{marginTop: 20}}>
              ...
              <div style={{display: "none"}}>
                ...
                >
                  {Array.from(new Set([...(record.gallery || []), ...(record.images || [])])).map(
                    (img: string, idx) => {
              ...
              <Space wrap size="middle">
                {Array.from(new Set([...(record.gallery || []), ...(record.images || [])]))
                  .slice(0, 5)
                  .map((img, idx) => {
