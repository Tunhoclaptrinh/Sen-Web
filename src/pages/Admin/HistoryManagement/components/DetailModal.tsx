...
const HistoryDetailModal: React.FC<HistoryDetailModalProps> = ({open, onCancel, record}) => {
  ...
  return (
    <Modal open={open} onCancel={onCancel} title={record.title} width={1000} footer={null}>
      <Tabs defaultActiveKey="content">
        <Tabs.TabPane tab="Nội dung" key="content">
          <div style={{marginBottom: 24}}>
            <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16}}>
              <Space size="large">
                <Space>
                  <UserOutlined style={{color: "#8c8c8c"}} /> <strong>{record.author || "Khuyết danh"}</strong>
                </Space>
                <Space>
                  <CalendarOutlined style={{color: "#8c8c8c"}} /> {dayjs(record.publishDate).format("DD/MM/YYYY")}
                </Space>
                <Tag color={record.isActive ? "green" : "red"}>{record.isActive ? "HIỂN THỊ" : "ĐÃ ẨN"}</Tag>
              </Space>
              <Space>
                <EyeOutlined /> {record.views || 0} lượt xem
              </Space>
            </div>
        ...
        <Tabs.TabPane tab="Thông tin bổ sung" key="info">
          <Descriptions bordered column={1}>
            <Descriptions.Item label="Tiêu đề bài viết">
